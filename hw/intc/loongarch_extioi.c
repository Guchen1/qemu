...
static void extioi_update_irq(LoongArchExtIOI *s, int irq_num, int level)
{
    ...
    if (level) {
        /* could i not declare these bitmaps as 'unsigned long name[BITS_TO_LONGS(N)]'?
         * For example, ext_sw_ipisr sw_ipisr[MAX_CORES][LS3A_INTC_IP] is a
         * two-dimensional array,and it has a specific meaning, memregion
         * options also restrict its size.
         */
        if (test_bit(irq_num, (void *)s->enable) == false) {
            return;
        }
        bitmap_set((void *)s->coreisr[cpu], irq_num, 1);
        /* search irq which is already set */
        found = find_first_bit((void *)&(s->sw_ipisr[cpu][ipnum]),
                               EXTIOI_IRQS);
        bitmap_set((void *)&(s->sw_ipisr[cpu][ipnum]), irq_num, 1);
        /* if there is no interrupt at present, then set the new irq */
        ...
        /* if the irq state is already set, then set it to low level */
        if (found == irq_num) {
            qemu_set_irq(s->parent_irq[cpu][ipnum], level);
        }
    }
...
static const VMStateDescription vmstate_ext_sw_ipisr = {
    ...
    .fields = (VMStateField[]) {
        VMSTATE_UINT8_ARRAY(irq, LoongArchExtSWIPISR, EXTIOI_IRQS / 8),
        VMSTATE_END_OF_LIST()
    }
};
